...
class NodePlugin(snapcraft.BasePlugin):
    ...
    def build(self):
        ...
        if not self.options.gulp_tasks and os.path.exists(
                os.path.join(self.builddir, 'package.json')):
            self.run(['npm', 'install', '-g'])
        if self.options.gulp_tasks:
            self.run(['npm', 'install', '-g', 'gulp-cli'])
            if os.path.exists(os.path.join(self.builddir, 'package.json')):
                self.run(['npm', 'install'])
            self.run([os.path.join(self.installdir, 'bin', 'gulp')] +
                     self.options.gulp_tasks)
